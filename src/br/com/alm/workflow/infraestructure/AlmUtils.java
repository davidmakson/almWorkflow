...
import br.com.alm.workflow.alm.AlmConnector;
...
public class AlmUtils {
	...
	private static Entity getSingleEntity(RestConnector conn, String entityUrl) throws Exception, JAXBException {
		...
		try {
			...
			System.out.println("Erro: " + postedEntityReturnedXml + "\n" + e.getMessage());
			return new Entity();
		}
	}
	...
	public static int getTestInstanceProduct(RestConnector conn, int mockTestInstanceId) {
		...
		try {
			...
			System.out.println(String.format("Erro ao obter produto para Instancia de Teste %1$d: %2$s", mockTestInstanceId,
					e.getMessage()));
		}
	...
	public static int getTestInstanceProduct(RestConnector conn, Entity testInstance) {
		...
		try {
			...
			System.out.println(String.format("Erro ao obter produto para Instancia de Teste %1$s: %2$s",
	...
	public static String getTestInstanceReleaseCycle(RestConnector conn, Entity testInstance) {
		...
		try {
			...
			System.out.println(String.format("Erro ao obter Ciclo de Release para Instancia de Teste %1$s: %2$s",
					AlmUtils.getEntityField(testInstance, Constants.ID), e.getMessage()));
		}
...
//			entity.getFields().getField().add(cycleIdField);
	...
	public void initParams() {
		...
		try (InputStream inputStream = new FileInputStream(new File("C:/workflowAlm/config/" + Constants.LOG_FILE))) {
			...
			System.out.println("Erro ao carregar arquivo properties de log: " + e.getMessage());
		}
		...
		try (InputStream inputStream = new FileInputStream(new File("C:/workflowAlm/config/" + Constants.APP_FILE))) {
			...
			this.password = prop.getProperty("PASSWORD_ALM");
	...
	/**
	 * Método que cria a conexão com a ALM e chama o método que atualiza o STATUS do test instance
	 * @param runID
	 * @param status
	 * @throws Exception
	 */
	public void updateTestInstanceStatus(final int testInstanceID, final String status) throws Exception {
		AlmConnector alm = new AlmConnector();
